...
\section{Type specialization}
  % Il est possible de définir un type général que l'on va par la suite pouvoir spécialiser et préciser. Chacun des schémas suivants décrivent une structure pouvant être utilisé pour réaliser ce projet.  Nous avons préféré ne pas les appliquer et avoir 4 grands types (``leisure book'', ``scientific book'', ``scientific journal'' and ``leisure periodicals'') afin de facilité la compréhension et d'éviter les erreurs.
  ...
  % Certains types peuvent être définit comme abstrait grâce à un attribut.  Concrètement, cela signifie qu'en rajoutant \verb|abstract="true"| dans la balise de définition du type, on pourra empêcher son instanciation dans un document et obliger ce type à être spécialisé pour être utilisé.
  ...
  % Dans les schémas suivant, les types en italique sont des types abstraits.
